...
class WaRPQCallback(TrainerCallback):
    ...
    def _compress_step(self, model: nn.Module, optimizer, state: TrainerState, save_tag: str):
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                ...
                if self.use_lagrange_alloc:
                    bits = bit_allocation_rd_lagrange(energies_after, sens, counts, avg_bit, self.rd_bit_choices)
                else:
                    bits = bit_allocation_rd(energies_after, sens, counts, avg_bit, self.rd_bit_choices)
                nnz_sum_layer = 0
                b_sum_layer = 0
                ...
                if self.save_payload:
                    ...
                    extra_meta = {"alpha": alpha, "beta": beta, "avg_bit": avg_bit, "is_I": is_I}
                ...
                for j in range(len(coeffs_E)):
                    ...
                    var_d = float((coeffs_orig[j].float() - preds[j].float()).var(unbiased=False).item()) if coeffs_orig[j].numel() > 0 else 0.0
        ...
        print(f"    - Frame: {'I' if is_I else 'P'} | Target nz ratio: {self.target_nz_ratio:.3f} | Actual p: {p:.4f}")
