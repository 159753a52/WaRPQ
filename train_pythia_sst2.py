...
class WaRPQCallback(TrainerCallback):
    ...
    def _current_frame_scale(self, step: int):
        ...
        if idx == 0:
            return self.i_frame_scale
        return self.p_frame_scale
    ...
    def _compress_step(self, model: nn.Module, optimizer, state: TrainerState, save_tag: str):
        ...
        frame_scale = self._current_frame_scale(state.global_step)
        avg_bit = max(1.5, min(6.0, self.avg_bitwidth * frame_scale))
        alpha = self.alpha_w2m * frame_scale
        beta = self.beta_m2w

        micro_eval_enabled = hasattr(self, "diag_micro_eval_size") and getattr(self, "diag_micro_eval_size") > 0 \
                            and hasattr(self, "calib_batch") and self.calib_batch is not None
            ...
            for name, param in model.named_parameters():
                ...
                if is_qkv:
                    ...
                    if self.enforce_target_nz:
                        mask = self._hard_topk_mask_qkv(E, v, self.target_nz_ratio)
                        E_masked = E * mask
                    else:
                        E_masked = E
                    ...
                    if optimizer is not None and self.momentum_bitwidth > 0 and (param in optimizer.state):
                        pstate = optimizer.state[param]
                        final_mask = keep_mask
                        if self.m2w_gate and ("exp_avg" in pstate) and isinstance(pstate["exp_avg"], torch.Tensor):
                            mtmp = pstate["exp_avg"].to(device=E.device, dtype=E.dtype)
                            ro = beta * torch.mean(mtmp.abs())
                            gate = (mtmp.abs() >= ro).to(dtype=E.dtype)
                            final_mask = final_mask * gate
                        if "exp_avg" in pstate and isinstance(pstate["exp_avg"], torch.Tensor):
                            mom = pstate["exp_avg"].to(device=E.device, dtype=E.dtype)
                            mom = mom * final_mask
                            mom_blocks, padm = blockify_flat(mom, self.block_size)
                            qmax = 2 ** (int(self.momentum_bitwidth) - 1) - 1
                            vmaxm = torch.quantile(torch.abs(mom_blocks), q=self.momentum_clip_pct, dim=-1, keepdim=True)
                            vmaxm = torch.clamp(vmaxm, min=1e-8)
                            qm = torch.clamp(torch.round(mom_blocks / (vmaxm / qmax)), min=-qmax, max=qmax)
                            deqm = qm * (vmaxm / qmax)
                            mom_rec = unblockify_flat(deqm, mom.numel()).reshape_as(mom)
                            pstate["exp_avg"].copy_(mom_rec)
                        if "exp_avg_sq" in pstate and isinstance(pstate["exp_avg_sq"], torch.Tensor):
                            mom2 = pstate["exp_avg_sq"].to(device=E.device, dtype=E.dtype)
                            mom2 = mom2 * final_mask
                            mom2_blocks, padm2 = blockify_flat(mom2, self.block_size)
                            qmax2 = 2 ** (int(self.momentum_bitwidth) - 1) - 1
                            vmaxm2 = torch.quantile(torch.abs(mom2_blocks), q=self.momentum_clip_pct, dim=-1, keepdim=True)
                            vmaxm2 = torch.clamp(vmaxm2, min=1e-8)
                            qm2 = torch.clamp(torch.round(mom2_blocks / (vmaxm2 / qmax2)), min=-qmax2, max=qmax2)
                            deqm2 = qm2 * (vmaxm2 / qmax2)
                            mom2_rec = unblockify_flat(deqm2, mom2.numel()).reshape_as(mom2)
                            pstate["exp_avg_sq"].copy_(mom2_rec)
                ...
                counts = [c.numel() for c in coeffs_E]
                ...
                if self.enforce_target_nz:
                    masks_wavelet = self._enforce_topk_in_wavelet(coeffs_E, sens, self.target_nz_ratio)
                    for j in range(len(coeffs_E)):
                        coeffs_E[j] = coeffs_E[j] * masks_wavelet[j]

                bits = bit_allocation_rd(energies, sens, counts, avg_bit, self.rd_bit_choices)
                ...
                if self.save_payload:
                    ...
                    extra_meta = {"alpha": alpha, "beta": beta, "avg_bit": avg_bit}
        ...
        print(f"    - Target nz ratio: {self.target_nz_ratio:.3f} | Actual p: {p:.4f}")
...
def main():
    ...
    parser.add_argument("--qkv_special_handling", action="store_true", default=True)
    ...
    parser.add_argument("--diag_micro_eval_size", type=int, default=0, help="Run a tiny eval on N samples right before/after each compression step (0=off).")
    parser.add_argument("--diag_layerwise_json", action="store_true", default=False, help="Dump per-layer stats JSON at each compression step.")
    ...
    print(f"Target nz ratio: {args.target_nz_ratio}, Enforce: {args.enforce_target_nz}, M2W gate: {args.m2w_gate}")
