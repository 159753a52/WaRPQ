...
class WaRPQCallback(TrainerCallback):
    ...
    def _compress_step(self,model:nn.Module,optimizer,state,tag:str):
        ...
        for name,param in model.named_parameters():
            ...
            _ = self._get_lr(optimizer, param)   # lr 如要统计仍可取，但不做预测更新
            pred = torch.zeros_like(w)           # 不使用 -lr * m / sqrt(v) 的预测项
            E_tilde = (w - base)                 # 不叠加历史误差反馈（EF）
            E = E_tilde                          # 统一用 E 参与后续统计/门控
            n_elem = E.numel()                   # ← 保留给后续统计（修复 NameError）
            mask_param = torch.ones_like(E_tilde, dtype=E_tilde.dtype)  # 默认全 1，占位
            ...
            rec_total = rec_E
            rec_totals[name] = rec_total.detach().cpu()
            if self.apply_to_model:
                param.data.copy_((base + rec_total).to(device=base.device, dtype=base.dtype))
...
#测试去除pred+ef的效果 去除掉的话会有0.01的loss值差距，这个是5个epoch后的累积效果