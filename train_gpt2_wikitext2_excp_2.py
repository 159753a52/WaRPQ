...
def kmeans_nonuniform_quantize(x:torch.Tensor,nbits:int,max_iters:int=12,assign_chunk:int=2_000_000)->QuantizedTensor:
    assert nbits>=2
    x=x.contiguous();flat=x.reshape(-1).to(torch.float32);device=flat.device
    nz_mask=flat!=0;nz=flat[nz_mask];K=(1<<nbits)-1
    if nz.numel()==0:
        centers=torch.zeros(K,device=device,dtype=torch.float32);idx=torch.zeros_like(flat,dtype=torch.uint8)
        return QuantizedTensor(tuple(x.shape),centers,_pack_nibbles(idx),nbits)
    q_lo=torch.quantile(nz,0.005);q_hi=torch.quantile(nz,0.995)
    if not torch.isfinite(q_lo): q_lo=nz.min()
    if not torch.isfinite(q_hi): q_hi=nz.max()
    if q_hi<=q_lo: q_lo=nz.min(); q_hi=nz.max() if nz.max()>nz.min() else (q_lo+1e-6)
    centers=torch.linspace(q_lo,q_hi,K,device=device,dtype=torch.float32)
    for _ in range(max_iters):
        counts=torch.zeros(K,device=device,dtype=torch.long);sums=torch.zeros(K,device=device,dtype=torch.float32)
        N=nz.numel()
        for s in range(0,N,assign_chunk):
            e=min(s+assign_chunk,N);vals=nz[s:e];d=(vals.unsqueeze(1)-centers.unsqueeze(0))**2
            a=torch.argmin(d,dim=1);counts+=torch.bincount(a,minlength=K);sums+=torch.bincount(a,weights=vals,minlength=K)
        nonempty=counts>0
        if nonempty.any(): centers[nonempty]=sums[nonempty]/counts[nonempty]
        empty=~nonempty
        if empty.any():
            lo,hi=nz.min(),nz.max();centers[empty]=torch.linspace(lo,hi,empty.sum(),device=device)
    idx=torch.zeros_like(flat,dtype=torch.uint8);nz_idx=torch.where(nz_mask)[0];N=nz_idx.numel()
    for s in range(0,N,assign_chunk):
        e=min(s+assign_chunk,N);sel=nz_idx[s:e];vals=flat[sel];d=(vals.unsqueeze(1)-centers.unsqueeze(0))**2;a=torch.argmin(d,dim=1);idx[sel]=(a+1).to(torch.uint8)
    return QuantizedTensor(tuple(x.shape),centers,_pack_nibbles(idx),nbits)
...
class ExCPConfig:
    ...
    apply_to_model:bool=True; momentum_policy:str="none"; qkv_bits:Optional[int]=None; qkv_group:int=128
    qkv_clip_pct:float=0.99; kmeans_iters:int=12; kmeans_qkv:bool=False
...
class ExCPCheckpointer(TrainerCallback):
    ...
    def on_step_end(self,args,state:TrainerState,control:TrainerControl,model=None,optimizer:Optional[Optimizer]=None,**k):
    ...
    def _compress_now(self,step:int,model:nn.Module,optimizer:Optional[Optimizer]):
        ...
        for name,p_obj in name_to_param.items():
            ...
            if self.cfg.enforce_target:
                ...
                rw=self.cfg.alpha/math.sqrt(v_mean+1e-12); keep_mask=(dW.abs()>rw); dWq=dW*keep_mask
            def _deq_E(Ek:torch.Tensor)->torch.Tensor:
                if _is_qkv(name):
                    bits=int(self.cfg.qkv_bits or self.cfg.nbits)
                    if self.cfg.kmeans_qkv: 
                        return kmeans_nonuniform_quantize(Ek,nbits=bits,max_iters=int(self.cfg.kmeans_iters)).dequantize(device=dev)
                    return quantize_pergroup_uniform(Ek,n_bits=bits,group=self.cfg.qkv_group,dim=-1,clip=self.cfg.qkv_clip_pct)
                return kmeans_nonuniform_quantize(Ek,nbits=int(self.cfg.nbits),max_iters=int(self.cfg.kmeans_iters)).dequantize(device=dev)
            rec_dW=_deq_E(dWq)
            payload_w[name]=kmeans_nonuniform_quantize(dWq,nbits=int(self.cfg.nbits),max_iters=int(self.cfg.kmeans_iters)).to_bytes() if not _is_qkv(name) or self.cfg.kmeans_qkv else {"shape":list(dWq.shape),"nbits":int(self.cfg.qkv_bits or self.cfg.nbits),"pergroup_uniform":True}
            if "m" in st:
                m1=st["m"]; mo_thr=self.cfg.beta*float(m1.abs().mean().item()); Mo=(m1.abs()>mo_thr)&keep_mask
                m1p=(m1*Mo).to(torch.float32); qt_m=kmeans_nonuniform_quantize(m1p,nbits=int(self.cfg.nbits),max_iters=int(self.cfg.kmeans_iters)); payload_o.setdefault(name,{})["m"]=qt_m.to_bytes()
                if "v" in st and isinstance(st["v"],torch.Tensor):
                    v2p=(st["v"]*Mo).to(torch.float32); qt_v=kmeans_nonuniform_quantize(v2p,nbits=int(self.cfg.nbits),max_iters=int(self.cfg.kmeans_iters)); payload_o[name]["v"]=qt_v.to_bytes()
...
def excp_reconstruct_to_state_dict(compressed_dir:str,upto_step:Optional[int]=None,init_weights_path:Optional[str]=None,device:Optional[torch.device]=None)->Dict[str,torch.Tensor]:
    ...
    for fn in xs:
        ...
        for name,qd in d["weights"].items():
            if isinstance(qd,dict) and qd.get("pergroup_uniform",False):
                continue
...
def main():
    ...
    if a.reconstruct_only:
        ...
        torch.save(state,out); print(f"Reconstructed state_dict saved to: {out}"); return
    ...
    cfg=ExCPConfig(save_dir=excp_dir,compress_every=a.excp_every,start_step=a.excp_start_step,nbits=a.excp_nbits,alpha=a.excp_alpha,beta=a.excp_beta,keep_init_state=a.excp_keep_init,apply_to_model=a.excp_apply_to_model,momentum_policy=a.excp_momentum_policy,target_nz=a.excp_target_nz,enforce_target=a.excp_enforce_target,qkv_bits=a.excp_qkv_bits,qkv_group=a.excp_qkv_group,qkv_clip_pct=a.excp_qkv_clip_pct,kmeans_iters=a.excp_kmeans_iters,kmeans_qkv=a.excp_kmeans_qkv)
    ...
    if a.single_shot:
        ...
        print(f"[ExCP OneShot] QKV quantizer = {'k-means' if a.excp_kmeans_qkv else 'per-group uniform'}, qkv_bits={a.excp_qkv_bits or a.excp_nbits}, target_nz={a.excp_target_nz}")
        kept,total=0,0; per_layer_err=[]
        opt_moments={}
        ...
        if src_opt_dir is not None:
            try:
                ...
                print(f"[ExCP OneShot] Loaded optimizer moments for {len(opt_moments)} params.")
        ...
        with torch.no_grad():
            for name,p in model.named_parameters():
                ...
                if a.excp_enforce_target:
                    ...
                    v_mean=float(opt_moments.get(name,{}).get("v",torch.zeros(1,device=dev)).mean().item()) if name in opt_moments and ("v" in opt_moments[name]) else 1.0
                    rw=a.excp_alpha/math.sqrt(v_mean+1e-12); keep_mask=(E.abs()>rw); E_kept=E*keep_mask
                def _kmeans_deq(x,nb): return kmeans_nonuniform_quantize(x,nbits=int(nb),max_iters=int(a.excp_kmeans_iters)).dequantize(device=dev)
                b_qkv=int(a.excp_qkv_bits or a.excp_nbits)
                if _is_qkv(name):
                    rec_E=_kmeans_deq(E_kept,b_qkv) if a.excp_kmeans_qkv else quantize_pergroup_uniform(E_kept,n_bits=b_qkv,group=a.excp_qkv_group,dim=-1,clip=a.excp_qkv_clip_pct)
                else:
                    rec_E=_kmeans_deq(E_kept,a.excp_nbits)
...
#稳定可对比版本
#python train_gpt2_wikitext2_excp_2.py --excp_apply_to_model --excp_start_step 400 --excp_every 400 --excp_nbits 4 --excp_target_nz 0.07 --excp_enforce_target --output_dir runs/excp_train